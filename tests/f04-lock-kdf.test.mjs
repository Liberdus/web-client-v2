...
test('application keeps legacy unlock support and hands the new record to atomic migration', async () => {
  ...
  assert.match(app, /await encryptAllAccounts\(\s*oldCredentials\?\.encryptionKey \|\| null,\s*nextLock\.encryptionKey,\s*nextLock\.record,\s*\)/);
  assert.doesNotMatch(app, /localStorage\.setItem\('lock', nextLock\.record\)/);
